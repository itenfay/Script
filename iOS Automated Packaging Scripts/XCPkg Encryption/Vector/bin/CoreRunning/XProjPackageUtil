...
# 使用说明：
# 编译project
# XProjPackingUtil <project directory> [-p <project name>] [-t <target name>] [-o <ipa output directory>]
#				   [-f <project configuration>] [-h <channel name>] [-m <min deployment>] [-c]
# 编译workspace
# XProjPackingUtil <project directory> -w -s <schemeName> [-f <project configuration>] [-m <min deployment>] [-c]
# 参数说明：
# -h		帮助
...
{
	echo '编译project'
	echo 'XProjPackingUtil <project directory> [-p <project name>] [-t <target name>] [-o <ipa output directory>]'
	echo '				   [-f <project configuration>] [-n <channel name>] [-m <min deployment>] [-c] [-h]'
	echo '编译workspace'
	echo 'XProjPackingUtil <project directory> -w -s <schemeName> [-f <project configuration>] [-m <min deployment>] [-c] [-h]'
	echo '参数说明：'
	echo '-h		帮助'
...
guide()
{
	echo 'XProjPackageUtil: Error, usage with "cmd -h"'
	exit
}

if [ $#  -lt 1 ];then
	echo 'XProjPackageUtil: Warning: enter the root directory of Xcode project.'
	exit 2
fi

if [ ! -d $1 ];then
	echo 'XProjPackageUtil: Warning: '$1' not a valid directory.'
	exit 2
fi
